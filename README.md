...
全平台同一套手势：任务池右滑进聚焦、左滑删除（要再确认）；聚焦右滑完成、左滑回任务池；完成左滑回任务池。
设置在标题栏齿轮里；归档在设置里单独打开。
...
其他：注册登录的多用户隔离（JWT），闲云 / 远山 / 归途 / 微光四套主题跟着账号走，
...
Docker 部署时 API 默认只绑在 `127.0.0.1:13000`，数据库不映射到宿主机，
都不对公网开放。公网访问一律走域名 + HTTPS。
...
不填域名就是本机自用，健康检查 `http://127.0.0.1:13000/health`。
...
首次打开在登录页填 API 地址（Docker 本机是 `http://127.0.0.1:13000`，
源码直接跑服务端仍是 `http://127.0.0.1:3000`，公网填你的域名），
